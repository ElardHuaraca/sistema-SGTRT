...
$(function () {
    ...
    $('#btn-update-create-project').on('click', () => $('#btn-sumbit-project').trigger('click'))
    ...
    $('#modalEditProject').on('show.bs.modal', function (e) {
        ...
        if (btn.attr('id') == 'btn-edit-project') {
            ...
            let project = projects.find(project => project.idproject == _id)
            setDataForm(project)
        } else {
...
$(function () {
    $('.form-update-project').on('submit', function (e) {
        e.preventDefault()
        let data = $(e.delegateTarget).serializeArray()
        let token = data[0].value
        let codigo_alp = data[1].value
        ...
        $.ajax({
            ...
            if (_id != null) {
                ...
                rowData[2] = data.name
                rowData[3] = `<button class="btn btn-warning" id="btn-edit-project" data-bs-toggle="modal" data-bs-target="#modalEditProject" value="${data.idproject}">
                                Editar
                            </button>`
                rowData[4] = `<button type="button" class="btn ${data.is_deleted === false ? 'btn-success' : 'btn-danger'} fs-6 btn-status-sow"  value="${data.idsow}">
                            ${data.is_deleted === false ? 'Activo' : 'Inactivo'}
                        </button>`
                $.dataTableInit.row(row).data(rowData)
                projects.map(function (project) {
                    if (project.idproject == _id) { project.name = data.name, project.idproject = data.idproject }
                    return project
                })
            } else {
                ...
                rowData[4] = `<button class="btn btn-success btn-update-project state-project-active" value="${data.idproject}">
                                Activo
                            </button>`
