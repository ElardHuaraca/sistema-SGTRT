...
$(function () {
    ...
    $('#btn-create-project').on('click', () => { _id = null; row = null })
    ...
    $('#modalEditProject').on('show.bs.modal', function (e) {
        ...
        if (btn.attr('id') == 'btn-edit-project') {
            ...
            let project = projects.find(project => project.idproyecto == _id)
            setDataForm(project)
        } else {
    ...
    $('.form-update-project').on('submit', function (e) {
        ...
        $.ajax({
            ...
            if (data.idproyecto == _id) {
                ...
                rowData[1] = data.idproyecto
                rowData[2] = data.nombre
                rowData[3] = `<button class="btn btn-warning" id="btn-edit-project" data-bs-toggle="modal" data-bs-target="#modalEditProject" value="${data.idproyecto}">
                                Editar
                            </button>`
                rowData[4] = `<button class="btn btn-danger btn-delete-project" data-bs-toggle="modal" data-bs-target="#modal-succes-confirmation" value="${data.idproyecto}">
                                Eliminar
                            </button>`
