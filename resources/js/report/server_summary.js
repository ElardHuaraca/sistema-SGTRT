...
$(function () {
    ...
    $('#modalEditServer').on('show.bs.modal', function (e) {
        ...
        var server = servers.filter(x => x.idserver == server_id)[0]
        ...
        assign_service = assign_services.filter(x => x.idserver == server.idserver)[0]
    ...
    $('#switchAditional').on('change', function () {
        ...
        if (checked) remove_disabled_on_assigned_service()
        else add_disabled_on_assigned_service_additional()
    })

    $('#switchLicenseServer').on('change', function () {
        checked = $(this).is(':checked')
        if (checked) remove_disabled_on_assigned_spla_service()
        else add_disabled_on_assigned_spla_service()
    })

    $('#switchLicenseWindows').on('change', function () {
        checked = $(this).is(':checked')
        switch_license_linux = $('#switchLicenseLinux')
        if (checked) switch_license_linux.prop('checked', false).attr('disabled', true)
        else switch_license_linux.removeAttr('disabled')
    })
    ...
    $('input[name="sow"]').on('keyup', function () {
        ...
        var sow_search = sows.filter(sow => {
            return sow.name.toLowerCase().includes($(this).val().toLowerCase()) ||
                sow.version.toLowerCase().includes($(this).val().toLowerCase()) || sow.type.toLowerCase().includes($(this).val().toLowerCase())
        })
        if (sow_search.length === 0) return

        values = (element) => { return element.version + ' ' + element.name + ' ' + element.type }
    ...
    $('#update_server').on('submit', function (e) {
        ...
        json = {
            ...
            'assign_spla_licences': {
                'SO': {
                    percentage: form.filter(x => x.name === 'reduction_so').length > 0 ? form.filter(x => x.name === 'reduction_so')[0].value : 0,
                    idspla: form.filter(x => x.name === 'SO' && x.value !== '').length > 0 ? form.filter(x => x.name === 'SO')[0].value : null,
                },
                'SQL Server': {
                    percentage: form.filter(x => x.name === 'reduction_sql').length > 0 ? form.filter(x => x.name === 'reduction_sql')[0].value : 0,
                    idspla: form.filter(x => x.name === 'SQL Server' && x.value !== '').length > 0 ? form.filter(x => x.name === 'SQL Server')[0].value : null,
                },
                'Remote Desktop': {
                    percentage: form.filter(x => x.name === 'reduction_remote').length > 0 ? form.filter(x => x.name === 'reduction_remote')[0].value : 0,
                    idspla: form.filter(x => x.name === 'Remote Desktop' && x.value !== '').length > 0 ? form.filter(x => x.name === 'Remote Desktop')[0].value : null,
                },
                'Office': {
                    percentage: form.filter(x => x.name === 'reduction_office').length > 0 ? form.filter(x => x.name === 'reduction_office')[0].value : 0,
                    idspla: form.filter(x => x.name === 'Office' && x.value !== '').length > 0 ? form.filter(x => x.name === 'Office')[0].value : null,
                }
            }
        }
        ...
        $.ajax({
            ...
            KEYS.forEach(key => {
                if (json.assign_spla_licences[key].idspla === null) {
                    spla = assign_splas.filter(x => x.type === key && x.idserver === JSON.parse(_id))[0]
                    if (spla !== undefined) SPLA_REMOVED.push(spla)
                }
            })

            assign_splas = assign_splas.filter(x => !SPLA_REMOVED.includes(x))
            response.assign_splas.forEach(x => {
                spla = assign_splas.filter(y => y.iddiscount === x.iddiscount)[0]
                if (spla === undefined) assign_splas.push(create_additional_spla(x))
                else assign_splas.map(z => { if (z.iddiscount === spla.iddiscount) update_additional_spla(z, x) })
            })
    ...
    function assigned_additionals_services_spla(idserver) {
        assign_spla_licences = assign_splas.filter(x => x.idserver == idserver)
        ...
        assign_spla_licences.forEach(function (assign_spla) {
            switch (assign_spla.type) {
                case 'SO':
                    ...
                    option = $(`select[name="SO"] option[value="${assign_spla.idspla}"]`)[0]
                ...
                case 'Remote Desktop':
                    ...
                    break;
                case 'Office':
                    option = $(`select[name="Office"] option[value="${assign_spla.idspla}"]`)[0]
                    $(option).prop('selected', true)
