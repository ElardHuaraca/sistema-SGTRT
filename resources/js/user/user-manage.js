...
$(function () {
    ...
    $('.form-update-user').on('submit', function (e) {
        ...
        $.ajax({
            ...
            if (data.idusuario == id) {
                ...
                rowData[3] = `<button type="button" class="btn btn-success fs-6 state-user-active" value="${data.idusuario}">
                                Activo
                            </button>`
                rowData[4] = `<button class="btn btn-warning" id="btn-edit-user" ata-bs-toggle="modal" data-bs-target=" #modalEditUser" value="${data.idusuario}">
                                Editar
                            </button>`
                rowData[5] = `<button class="btn btn-danger" id="btn-delete-user" value="${data.idusuario}">
                                Eliminar
                            </button>`
                $.dataTableInit.row.add(rowData).draw()
                users.push({
                    'idusuario': data.idusuario, 'nombres': data.nombres,
                    'apellidos': data.apellidos, 'usuario': data.usuario,
                    'correo': data.correo, 'telefono': data.telefono, 'rol': data.rol
                })
            }
        }).catch(function (data) {
            $('#btn-close-loading').trigger('click')
    ...
    $('#delete-data').on('click', function () {
        ...
        $.ajax({
            ...
            users = users.filter(function (user) { return user.idusuario != _id })
        }
        ).catch(function (data) {
            $('#btn-close-loading').trigger('click')
            $('#btn-succes-error').trigger('click')
