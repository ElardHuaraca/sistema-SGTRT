...
/*!**************************************************!*\
  !*** ./resources/js/maintenance/licence_spla.js ***!
  \**************************************************/
var action = null;
var row = null;
var id = null;
$(function () {
  ...
  $('#btn-update-create-licence_spla').on('click', function () {
    $('#btn-sumbit-licence_spla').trigger('click');
  });
  $('#modalCreateEditLicenceSpla').on('show.bs.modal', function (event) {
    ...
    if (relatedTarget.hasClass('btn-edit-licence-spla')) $('#staticBackdropLabel').text('Editar Licencia SPLA');else $('#staticBackdropLabel').text('Crear Licencia SPLA');

    if (row !== null) {
      var licence_spla = licences.filter(function (x) {
        return "".concat(x.idspla) === relatedTarget.val();
      })[0];
      $('.form-update-create-licence_spla').find('input,select').each(function (_index, element) {
  ...
  $('.form-update-create-licence_spla').on('submit', function (e) {
    ...
    $.ajax({
      ...
      if (action === 'create') {
        ...
        row_data[4] = "$ ".concat(response.cost);
        row_data[5] = "<button class=\"btn btn-warning btn-edit-licence-spla\" data-bs-toggle=\"modal\"\n                                data-bs-target=\"#modalCreateEditLicenceSpla\" value=\"".concat(response.idspla, "\">Editar</button>");
        row_data[6] = " <button class=\"btn btn-success btn-status-licence-spla\"\n                                value=\"".concat(response.idspla, "\">Activo</button>");
