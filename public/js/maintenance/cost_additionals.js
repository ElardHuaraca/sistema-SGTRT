...
/*!******************************************************!*\
  !*** ./resources/js/maintenance/cost_additionals.js ***!
  \******************************************************/
var _id = null;
var _row = null;
var _idproject = null;
...
$(function () {
  ...
  $('#modalUpdateFourwall').on('show.bs.modal', function (event) {
    ...
    var fourwall = fourwalls.filter(function (x) {
      return x.idfourwall === JSON.parse($(relatedTarget).attr('value'));
    })[0];
    /* set data on inputs */
  ...
  $('#modalUpdateNexus').on('show.bs.modal', function (event) {
    var relatedTarget = event.relatedTarget;
    var nexu = nexus.filter(function (x) {
      return x.idnexus === JSON.parse($(relatedTarget).attr('value'));
    })[0];
    /* set data on inputs */
  ...
  $('#modalUpdateHp').on('show.bs.modal', function (event) {
    var relatedTarget = event.relatedTarget;
    var hp = hps.filter(function (x) {
      return x.idhp === JSON.parse($(relatedTarget).attr('value'));
    })[0];
    /* set data on inputs */
  ...
  $('#modalUpdateFourwall').on('hide.bs.modal', function () {
    $('input[name="codigo_alp"').attr('disabled', true);
  });
  $('#modalUpdateNexus').on('hide.bs.modal', function () {
    $('input[name="codigo_alp"').attr('disabled', true);
  });
  $('#modalUpdateHp').on('hide.bs.modal', function () {
    $('input[name="codigo_alp"').attr('disabled', true);
  });
  ...
  $('#btn-update-fourwall').on('click', function () {
    $('#btn-sumbit-fourwall').trigger('click');
  });
  $('#btn-update-nexus').on('click', function () {
    $('#btn-sumbit-nexus').trigger('click');
  });
  $('#btn-update-hp').on('click', function () {
  ...
  $('.form-update-fourwall').on('submit', function (e) {
    ...
    $.ajax({
      ...
      type: 'PUT',
      data: {
        'equipment': form.filter(function (x) {
          return x.name == 'equipment_fourwall';
        })[0].value,
        'serie': form.filter(function (x) {
          return x.name == 'serie_fourwall';
        })[0].value,
        'cost': form.filter(function (x) {
          return x.name == 'cost_fourwall';
        })[0].value,
      ...
      rowData[6] = "".concat(response_date_start.toLocaleDateString('es-CL')).replaceAll('-', '/');
      rowData[7] = response.date_end === null ? 'N.E.' : "".concat(response_date_end.toLocaleDateString('es-CL')).replaceAll('-', '/');
      $.dataTableInit.row(_row).data(rowData);
      fourwalls = fourwalls.map(function (x) {
        if (x.idfourwall === _id) {
  ...
  $('.form-update-nexus').on('submit', function (e) {
    ...
    $.ajax({
      ...
      type: 'PUT',
      data: {
        'network_point': form.filter(function (x) {
          return x.name == 'point_red_nexus';
        })[0].value,
        'cost': form.filter(function (x) {
  ...
  $('.form-update-hp').on('submit', function (e) {
    ...
    $.ajax({
      ...
      data: {
        'equipment': form.filter(function (x) {
          return x.name == 'equip_hp';
        })[0].value,
        'serie': form.filter(function (x) {
          return x.name == 'serie_hp';
        })[0].value,
        'cost': form.filter(function (x) {
          return x.name == 'cost_hp';
        })[0].value,
        'date_start': form.filter(function (x) {
          return x.name == 'date_start';
        })[0].value,
  ...
  $('#table-resources-it').on('click', '.btn-delete-fourwall', function () {
    $('#btn-succes-confirmation').trigger('click');
    _id = $(this).attr('value');
    _row = $(this).parents('tr')[0];
    _type = 'fourwall';
  });
  $('#table-resources-it').on('click', '.btn-delete-nexus', function () {
    $('#btn-succes-confirmation').trigger('click');
    _id = $(this).attr('value');
    _row = $(this).parents('tr')[0];
    _type = 'nexus';
  });
  $('#table-resources-it').on('click', '.btn-delete-hp', function () {
    $('#btn-succes-confirmation').trigger('click');
    _id = $(this).attr('value');
    _row = $(this).parents('tr')[0];
    _type = 'hp';
  ...
  $('#btn-succes-confirmation').on('click', function () {
    $('#title_action_perform').text('¿Está seguro de eliminar este registro?');
  });
  ...
  function switchDelete(response) {
    switch (_type) {
      case 'fourwall':
        ...
        fourwalls = fourwalls.map(function (x) {
          return x.idfourwall === _id ? response : x;
        });
        break;
